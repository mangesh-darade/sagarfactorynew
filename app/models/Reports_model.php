...
//sun 
    ...
    {
        $res = $this->db->query("SELECT SUM(`quantity`) as qty,product_code FROM " . $this->db->dbprefix('sale_items') . " WHERE product_code=".$p_c);

        if ($res->num_rows() > 0) {
            foreach (($res->result()) as $row) {
                $data[] = $row;
            }
        //    echo $res->row(0)->product_code;
        //    exit;
            // return $res;
            return $data;
        }
    ...
    // SELECT SUM(`quantity`),customer_id FROM `sma_sale_items` WHERE sma_sales.id=sma_sale_items.sale_id and `product_code`=1020 AND `customer_id`=1
...
//sun
    ...
    function load_product_varient_sale_data($Data, $startpoint = '', $per_page = '', $search = '') {
        $query = "select p.id as product_id, p.name, p.code, p.price, c.name as cat_name, b.name as brand_name, p.quantity as qty, (p.quantity * p.cost) as product_cost ";
...
//     print_r($this->db->last_query());
    ...
    public function payment_option() {
        ...
        $getpayment_option = $this->db->select('supercash_amount as supercash,authorize,instamojo,ccavenue,credit_card as CC,debit_card as DC,gift_card,neft as NEFT,paytm_opt as Paytm,UPI_QRCODE,google_pay as Googlepay,swiggy,zomato,ubereats,magicpin,complimentary as complimentry,paynear as paynear,payumoney,stripe')
    ...
    {
        $qry = "SELECT DISTINCT paid_by FROM `sma_payments`";
...
//26-09-2020
    ...
    /**
     * Get Customer Deposit Ledger Records d
     */
