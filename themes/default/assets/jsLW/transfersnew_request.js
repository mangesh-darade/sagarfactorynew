...
$(document).on('click', '#editItem', function () {
    ...
    toitems[item_id].row.fup = 1,
            //toitems[item_id].row.quantity = parseFloat($('#warh1qty').val()),//variant Quantity to warehouse 
            //toitems[item_id].row.getstock_2 = parseFloat($('#warh2qty').val()),//variant Quantity from warehouse 
    ...
    if (localStorage.getItem('toitems')) {
       ...
       sortedItems = _.sortBy(toitems, function(o){return [parseInt(o.order)]; });
        ...
        $.each(sortedItems, function () {
            ...
            var quantity = item.row.quantity;//=='')?item.options[0].total_quantity:'';
...
//            }
            ...
            if (item.options !== false) {
                $.each(item.options, function () {
                    if (this.id == item_option && base_quantity > item_aqty) {
